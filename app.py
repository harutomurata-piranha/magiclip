...
- 序盤は短いカットで勢いを出し、一番の見どころだけ長めに見せて印象を残す
...
- 並び順：シーンは時系列順（startの昇順）。物語が伝わる自然な流れを最優先し、場面を前後に入れ替えない。同じ映像の重複は入れない
- start/end は 0以上{duration:.1f}以下
- 長さは結果でよい。無駄（長い無音・聞き取れない崩れ・冗長な繰り返し・どうでもいい雑談）だけ削り、見どころや締めは削らない（無理に短く絞り込まない）
...
def analyze_content(annotated, duration):
    ...
    prompt = (f"あなたはMagiClipの動画編集ディレクターです。次の{duration:.1f}秒の動画の文字起こし"
              ...
              "2. 見どころ・価値のある場面（必ず残す所。複数あればすべて挙げる。秒数も）\n"
              "3. 終わりに最適な場面（オチ・まとめ・余韻が残る一言。秒数）\n"
              "4. 人間味が出ている場面（声・表情・言葉。残したい所。秒数）\n"
              "5. 明らかに不要で削れる部分（長い無音・冗長な繰り返し・どうでもいい雑談。秒数で。少しでも価値があれば挙げない）")
    try:
        msg = anthropic_client.messages.create(
